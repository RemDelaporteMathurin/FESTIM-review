...
exact_solution_left = 1 + sp.sin(2*sp.pi*(x + 0.25)) + sp.cos(2*sp.pi*y) # exact solution
...
form = u * v * my_model.mesh.dx - exact_solution_left * v * my_model.mesh.dx(1) - exact_solution_right * v * my_model.mesh.dx(2)
...
for i, profile in enumerate(profiles):
    ...
    computed_values = [my_model.h_transport_problem.mobile.post_processing_solution(x, y) for x, y in zip(points_x, points_y)]
