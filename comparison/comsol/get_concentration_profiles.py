...
def extract_concentration_profile(filename, dataname, export_dataname=None):
    ...
    if filename.endswith(".xdmf"):
        ...
        source = XMLUnstructuredGridReader(FileName=[filename])
    # get animation scene
    ...
    # save data
    SaveData(
        f"{export_dataname}_profile.csv",
        proxy=plotOverLine1,
        PointDataArrays=["arc_length", dataname, "vtkValidPointMask"],
    )


extract_concentration_profile("./inventory.vtu", "Color", "inventory")
extract_concentration_profile("./cm.vtu", "Color", "mobile_concentration")
