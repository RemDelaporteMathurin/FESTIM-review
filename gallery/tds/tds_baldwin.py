...
def run_tds(bc_type: str):
    """Runs a FESTIM model with a specific BC type

    Args:
        bc_type (str): 'Sieverts', 'Recombination flux' or 'DSC'

    Raises:
        ValueError: if the bc_type is unknown

    Returns:
        F.DerivedQuantities: the derived quantities from the simulation
    """
    model = F.Simulation()
